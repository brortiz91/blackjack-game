...
class Blackjack(object):
	def __init__(self):
		...
		#Pass the player and the dealer 2 cards each
	...
	def hitDealer(self):
		...
		if playerPoints > 21:
			return "WELP! YOU BUSTED! YOU LOSE!"
		else:
			...
			if dealerPoints > 21:
				...
				return "The dealer has gotten a higher score than you! YOU LOSE!"
			elif dealerPoints < playerPoints and playerPoints <= 21:
				return "You have gotten a better score than the dealer! YOU WIN!"
			elif dealerPoints == playerPoints:
				if self._player.hasBlackjack() and not self._dealer.hasBlackjack():
					return "HOLY MOLY, YOU GOT A FREAKIN' BLACKJACK! (btw, you win lol)"
				elif not self._player.hasBlackjack() and self._dealer.hasBlackjack():
					return "Uh-Oh.... the dealer has gotten a BLACKJACK and you got rekt!"
				else:
					return "WELP! IT'S A PUSH!"	